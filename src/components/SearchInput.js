...
class SearchInput extends React.Component {
  ...
  handleInputFirstClick = () => {
    this.setState({offset: 0},()=>{this.handleInputClick()})
  }
  ...
  handlePagePlus = (e) => {
    ...
    this.setState({offset: offset+1},()=>{this.handleInputClick()})
  }
  ...
  onSelectSido = (option) => {
    ...
    const chungnamOptions = [
      '공주시','보령시','아산시','서산시','논산시','계룡시','당진시','금산군','부여군','서산군','청양군','홍성군','예산군','태안군'
    ,'모두 선택'];
    const deguOptions = [
    ...
    const gyeongbukOptions = [
      '경주시','김천시','안동시','구미시','영주시','영천시','상주시','문경시','경산시','군위군','의성군','청송군','영양군','영덕군','청도군','고령군','성주군','칠곡군','예천군','봉화군','울진군','울릉군'
    ,'모두 선택'];
    const changwonOptions = [
    ...
    const gyunggiOptions = [
      '의정부시','부천시','광명시','평택시','동두천시','과천시','구리시','남양주시','오산시','시흥시','군포시','의왕시','하남시','파주시','이천시','안성시','김포시','화성시','광주시','양주시','포천시','여주시','연천군','가평군','양평군'
    ,'모두 선택'];
    const inchunOptions = [
  ...
  render() {
    ...
    const sidoOptions = [
      '서울','경기도','경기도 수원시','경기도 안양시','경기도 고양시','경기도 용인시','경기도 안산시','강원도','충청북도','충청북도 청주시','충청남도','충청남도 천안시','전라북도','전라북도 전주시','전라남도','경상북도','경상북도 포항시','경상남도','경상남도 창원시','부산','대구','인천','광주','대전','울산','제주특별자치도','세종특별자치시'
    ,'모두 선택'];
    const subjectOptions = [
      '수학','영어','국어','과학'
    ,'모두 선택'];
    const gradeOptions = [
      '고등학교 3학년','고등학교 2학년','고등학교 1학년','중학교 3학년','중학교 2학년','중학교 1학년','초등학교'
    ,'모두 선택'];
    const sortOptions = [
      '기본 정렬','별점 높은순','좋아요 많은 순','가격 높은순','가격 낮은순'
    ];

    let minus = <div className = "none"></div>
    if (this.state.offset > 0) {
      minus = <button className="pageButton" onClick={this.handlePageMinus}>&#60;</button>
    }else{
      minus = <div className = "none"></div>
    }

    let plus = <div className = "none"></div>
    if (this.state.rawData.length == 71) {
      plus = <button className="pageButton" onClick={this.handlePagePlus}>&#62;</button>
    }else{
      plus = <div className = "none"></div>
    }
      ...
      <div className = "searchBody">
        <div className="searchInputField">
          ...
          <div className = "searchInputFieldCol">
            <Dropdown options={sidoOptions} value={this.state.sidoLabel} onChange={this.onSelectSido} placeholder="시,도" 
              className='dropdown'menuClassName='dropdownMenu'/>
            <Dropdown options={this.state.gunguOptions} value={this.state.gunguLabel} onChange={this.onSelectGungu} placeholder="군,구" 
              className='dropdown'menuClassName='dropdownMenu'/>
            <Dropdown options={subjectOptions} value={this.state.subjectLabel} onChange={this.onSelectSub} placeholder="과목" 
              className='dropdown'menuClassName='dropdownMenu'/>
            <Dropdown options={gradeOptions} value={this.state.gradeLabel} onChange={this.onSelectGrade} placeholder="학년" 
              className='dropdown'menuClassName='dropdownMenu'/>
          </div>
            <div className="searchInputFieldCol">
            ...
            <input className='searchInputFieldTextInput' type="text" onKeyUp={this.handleKeyUp} value={this.state.hakwonName} onChange={this.handleChangeInput} name="hakwonName" placeholder="학원이름을 입력해 주세요" />
        ...
        <div className="resultField">
          <div className="hakwonResult">
            <Dropdown options={sortOptions} value={this.state.sort} onChange={this.onSelectSort} placeholder="정렬기준" 
              className='dropdown'menuClassName='dropdownMenu'/>
            {pageShow.map((pageShow) => {
